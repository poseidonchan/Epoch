...
final class AppStoreSettingsTests: XCTestCase {
    func testPreferredBackendDefaultsToCodexAppServer() {
        let suiteName = "EpochCoreTests.\(UUID().uuidString)"
    ...
    func testApplyHubPairingQRCodeSavesGatewaySettingsFromValidPayload() async throws {
        ...
        let raw = "epoch://pair?v=1&ws=\(encodedWS)&token=tok_123&serverId=srv_1"
    ...
    func testApplyHubPairingQRCodeRejectsMissingTokenWithoutMutatingSettings() async {
        ...
        do {
            try await store.applyHubPairingQRCode("epoch://pair?v=1&ws=\(encodedWS)")
    ...
    func testApplyHubPairingQRCodeAttemptsConnectionAfterSaving() async throws {
        ...
        try await store.applyHubPairingQRCode("epoch://pair?v=1&ws=\(encodedWS)&token=tok_abc")
    ...
    func testSetProjectPermissionLevelSyncsCodexProjectUpdateAndMutatesLocalState() async throws {
        ...
        try await waitUntil(timeoutSeconds: 1.0) {
            capturedMethod == "epoch/project/update"
        }
