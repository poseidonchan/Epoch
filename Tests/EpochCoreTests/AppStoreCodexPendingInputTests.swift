...
final class AppStoreCodexPendingInputTests: XCTestCase {
    ...
    func testRespondToCodexPromptKeepsPromptWhenCodexClientUnavailable() async throws {
        ...
        store.respondToCodexPrompt(
            sessionID: sessionID,
            requestID: .string("req_prompt_retry"),
            answers: ["epoch_plan_implementation_decision": ["Yes, implement this plan"]]
        )
    ...
    func testRespondToImplementConfirmationPromptKeepsPlanModeEnabledForFeedback() async throws {
        ...
        store.respondToCodexPrompt(
            sessionID: sessionID,
            requestID: .string("req_impl_feedback"),
            answers: ["epoch_plan_implementation_decision": ["Use stricter constraints first"]]
        )
    ...
    func testCodexQueuedInputsReorderPersistsAcrossStoreRecreate() {
        let suiteName = "EpochCoreTests.CodexQueuedInput.\(UUID().uuidString)"
    ...
    func testReorderCodexQueuedInputsPersistsExplicitIDOrder() throws {
        let suiteName = "EpochCoreTests.CodexQueuedInput.ReorderByIDs.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            return XCTFail("Failed to create suite-backed defaults.")
        }
        defaults.removePersistentDomain(forName: suiteName)
    ...
    func testSetPermissionLevelInCodexModeSyncsSessionAndProjectDefaults() async throws {
        ...
        try await waitUntil(timeoutSeconds: 1.0) {
            Set(calls.map(\.method)).isSuperset(of: ["epoch/session/update", "epoch/project/update"])
        }

        let sessionParams = try XCTUnwrap(
            calls.first(where: { $0.method == "epoch/session/update" })?.params?.objectValue
        )
        ...
        let projectParams = try XCTUnwrap(
            calls.first(where: { $0.method == "epoch/project/update" })?.params?.objectValue
        )
        ...
        await store._receiveCodexServerRequestForTesting(
            CodexRPCRequest(
                ...
                params: .object([
                    ...
                    "questions": .array([
                        .object([
                            "id": .string("epoch_plan_implementation_decision"),
                            "question": .string(""),
                            "isOther": .bool(true),
                            "options": .array([
                                .object([
