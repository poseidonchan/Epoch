...
final class HubIntegrationTests: XCTestCase {
    private func loadLocalHubToken() -> String? {
        // Local dev convenience only. This test is skipped unless LABOS_HUB_INTEGRATION=1.
        let url = URL(fileURLWithPath: NSHomeDirectory())
            .appendingPathComponent(".labos/config.json")
    ...
    private func makeIsolatedDefaults() -> UserDefaults {
        let suite = "LabOS.HubIntegrationTests.\(UUID().uuidString)"
    ...
    func testCreateSessionRecoversForLocalOnlyProjectWhenGatewayConfigured() async throws {
        guard ProcessInfo.processInfo.environment["LABOS_HUB_INTEGRATION"] == "1" else {
            throw XCTSkip("Set LABOS_HUB_INTEGRATION=1 to run Hub integration tests.")
        }

        guard let token = loadLocalHubToken(), !token.isEmpty else {
            throw XCTSkip("Missing ~/.labos/config.json token; cannot run integration test.")
        }
