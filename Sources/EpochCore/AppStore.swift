...
private struct HubPairingPayload: Equatable {
    ...
    static func parse(_ raw: String) throws -> HubPairingPayload {
        ...
        guard components.scheme?.lowercased() == "epoch",
...
public final class AppStore: ObservableObject {
    internal enum DefaultsKey {
        static let gatewayDeviceID = "Epoch.gateway.deviceID"
        static let gatewayWSURL = "Epoch.gateway.wsURL"
        static let gatewayToken = "Epoch.gateway.token"
        static let backendEngine = "Epoch.backend.engine"
        static let hpcPartition = "Epoch.hpc.partition"
        static let hpcAccount = "Epoch.hpc.account"
        static let hpcQos = "Epoch.hpc.qos"
        static let runCompletionNotificationsEnabled = "Epoch.notifications.runCompletion.enabled"
        static let openAIVoiceTranscriptionModel = "Epoch.openai.voice.transcriptionModel"
        static let openAIVoiceTranscriptionPrompt = "Epoch.openai.voice.transcriptionPrompt"
        static let openAIOcrModel = "Epoch.openai.ocr.model"
        static let codexPromptDraftPrefix = "Epoch.codexPromptDraft"
        static let codexTrajectoryDurations = "Epoch.codexTrajectoryDurations"
        static let codexQueuedInputPrefix = "Epoch.codexQueuedInput"
    }
    ...
    internal func persistProjectsCache(_ projects: [Project]) {
        ...
        defaults.set(data, forKey: "epoch.cachedProjects")
    }

    internal func persistSessionsCache() {
        guard let data = try? JSONEncoder().encode(sessionsByProject) else { return }
        defaults.set(data, forKey: "epoch.cachedSessions")
    }

    private func loadCachedProjectsIfEmpty() {
        guard projects.isEmpty else { return }
        guard let data = defaults.data(forKey: "epoch.cachedProjects"),
              let cached = try? JSONDecoder().decode([Project].self, from: data),
              !cached.isEmpty
        else { return }
        projects = cached

        if let sessData = defaults.data(forKey: "epoch.cachedSessions"),
    ...
    public func connectGateway() async {
        ...
        let client = GatewayClient(wsURL: wsURL, token: gatewayToken, deviceID: gatewayDeviceID, deviceName: "Epoch iPhone")
    ...
    internal func ensureGatewayConnectedForChat() async -> Bool {
        ...
        let task = Task { @MainActor [weak self] in
            ...
            if let existing = self.gatewayClient {
                ...
                let newClient = GatewayClient(wsURL: wsURL, token: self.gatewayToken, deviceID: self.gatewayDeviceID, deviceName: "Epoch iPhone")
    ...
    private static let codexImageBridgeDirectoryName = "epoch-codex-image-bridge"
    ...
    internal func requestCodex<Params: Encodable>(method: String, params: Params) async throws -> CodexRPCResponse {
        ...
        guard let codexClient, isCodexConnected else {
            throw NSError(domain: "Epoch", code: -1, userInfo: [NSLocalizedDescriptionKey: "Codex client is not connected"])
        }
    ...
    internal func decodeCodexResult<T: Decodable>(_ result: JSONValue?, key: String? = nil) throws -> T {
        ...
        if let key {
            guard let object = result?.objectValue, let nested = object[key] else {
                throw NSError(domain: "Epoch", code: -1, userInfo: [NSLocalizedDescriptionKey: "Missing codex result key \(key)"])
            }
            value = nested
        } else {
            guard let result else {
                throw NSError(domain: "Epoch", code: -1, userInfo: [NSLocalizedDescriptionKey: "Missing codex result payload"])
            }
            value = result
        }
    ...
    private func decodeCodexSkillsEntries(_ result: JSONValue?) throws -> [CodexSkillsListEntry] {
        ...
        // - { "entries": [SkillsListEntry] } (Epoch compatibility)
    ...
    private static let codexInjectedContextMarkerPrefix = "[EPOCH_"
    ...
    private static func codexPromptKind(from params: [String: JSONValue]) -> String {
        ...
        if questionID == "epoch_plan_implementation_decision" {
    ...
    private func applyE2EFixturesIfNeeded() {
        ...
        guard env["EPOCH_E2E_FIXTURE_PLAN_PROMPT_FLOW"] == "1" else { return }
    ...
    internal func decodeGatewayPayload<T: Decodable>(_ payload: [String: JSONValue]?, key: String) throws -> T {
        guard let payload, let value = payload[key] else {
            throw NSError(domain: "Epoch", code: -1, userInfo: [NSLocalizedDescriptionKey: "Missing payload key \(key)"])
        }
        let data = try gatewayJSONEncoder.encode(value)
        return try gatewayJSONDecoder.decode(T.self, from: data)
    }
    ...
    internal func decodeGatewayPayloadObject<T: Decodable>(_ payload: [String: JSONValue]?) throws -> T {
        guard let payload else {
            throw NSError(domain: "Epoch", code: -1, userInfo: [NSLocalizedDescriptionKey: "Missing payload object"])
        }
    ...
    private static let planImplementationDecisionQuestionID = "epoch_plan_implementation_decision"
