...
test("resolveRuntimeCommandCwd falls back to provided default cwd", () => {
  const cwd = resolveRuntimeCommandCwd({
    ...
    projectRoot: "/tmp/labos/projects/proj_1",
    rawCwd: undefined,
    fallbackCwd: "/tmp/labos/projects/proj_1",
  });
  assert.equal(cwd, path.resolve("/tmp/labos/projects/proj_1"));
});
...
test("resolveRuntimeCommandCwd resolves projects/* relative cwd against workspace root", () => {
  ...
  assert.equal(cwd, path.resolve("/tmp/labos/projects/proj_1/runs/run_1"));
});
...
test("resolveRuntimeCommandCwd resolves non-project relative cwd against project root", () => {
  ...
  assert.equal(cwd, path.resolve("/tmp/labos/projects/proj_1/runs/run_2"));
});
