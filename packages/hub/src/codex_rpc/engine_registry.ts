...
import { PiAgentEngineAdapter } from "./engines/pi_adapter.js";
import type { CodexEngineSession } from "./engines/types.js";

export type CodexEngineName = "pi" | "codex-app-server";

const DEFAULT_ENGINE: CodexEngineName = "pi";

export class CodexEngineRegistry {
  private readonly config: HubConfig | null;
  private readonly stateDir: string;

  private readonly piEngine: PiAgentEngineAdapter;
  ...
  constructor(opts: { config: HubConfig | null; stateDir: string }) {
    ...
    this.piEngine = new PiAgentEngineAdapter({ config: this.config });
  }
...
export function normalizeEngineName(raw: unknown): CodexEngineName | null {
  ...
  if (normalized === "pi" || normalized === "pi-adapter") return "pi";
