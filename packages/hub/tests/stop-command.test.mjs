...
test("hub stop is safe when daemon not running", async () => {
  ...
  assert.match(stdout, /already stopped|No running Epoch server daemon found/i);
});
...
test("hub stop cleans stale pid metadata", async () => {
  ...
  const { stdout } = await runHub(["stop"], { EPOCH_STATE_DIR: stateDir });
  assert.match(stdout, /already stopped|stopped|No running Epoch server daemon found/i);
