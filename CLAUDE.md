...
Epoch is an iPhone-first SwiftUI app backed by a Node.js gateway daemon (Hub) and an optional HPC Bridge. The iPhone app talks to the Hub over WebSocket using a custom request/response/event framing protocol. The Hub orchestrates an AI agent loop and persists state as append-only JSONL transcripts mirrored to SQLite.

## Commands

### iOS App

Build and run via Xcode (scheme: `EpochApp`, target: iOS simulator or device):

```bash
xcodegen generate          # Regenerate Epoch.xcodeproj from project.yml
swift test                 # Run unit tests (EpochCoreTests)
```
...
E2E tests (`EpochE2ETests`) are UI tests — run from Xcode with a simulator, not `swift test`.
...
node packages/hub/dist/cli.js start     # Start daemon (background, logs → ~/.epoch/hub.log)
...
Hub (Fastify + ws)
  ...
  └─ JSONL transcripts  (~/.epoch/projects/<id>/sessions/<id>.jsonl, canonical)
...
- **`storage/layout.ts`**: File path helpers for `~/.epoch/projects/<id>/sessions/`, uploads, cache, generated dirs.
...
TypeBox schema (`src/schema.ts`) defines all wire types shared between Hub and HPC Bridge. `dist/schema.json` is consumed by `tools/codegen-swift` to generate `Sources/EpochCore/Generated/`.
...
- Unit tests (`EpochCoreTests`) use `AppStore(bootstrapDemo: false)` with `MockBackendClient` — no real network needed.
- E2E tests (`EpochE2ETests`) use `XCUIApplication` and require a running simulator.
- `project.yml` (XcodeGen) is the source of truth for the Xcode project; never edit `Epoch.xcodeproj` by hand.
